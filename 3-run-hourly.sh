...
function install_crontab() {
  if [[ -z "$(crontab -l)" ]] || [[ ! -z $(crontab -l | grep '# Updated by brewery-kit') ]]; then
    ...
    (
      ...
      echo 'SHELL=/bin/bash'
      echo '@reboot (docker run --rm '${VERSION}' cat /home/docker/brewery_kit_tools/2-start-jobs.sh) | bash'
      echo '0 * * * * (docker run --rm '${VERSION}' cat /home/docker/brewery_kit_tools/3-run-hourly.sh) | bash'
    ) | crontab -
  fi
}
